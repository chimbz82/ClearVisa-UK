...
const FAQItem: React.FC<FAQItemProps> = ({ question, answer, isOpen, onToggle }) => (
  <div className="bg-white rounded-2xl mb-4 overflow-hidden border border-slate-100 shadow-sm transition-all hover:shadow-md">
    <button onClick={onToggle} className="w-full flex items-center justify-between p-6 text-left focus:outline-none group">
      <span className="body-m font-bold text-navy group-hover:text-accent transition-colors">{question}</span>
    ...
    <div className={`overflow-hidden transition-all duration-300 ${isOpen ? 'max-h-[600px]' : 'max-h-0'}`}>
      <div className="p-6 pt-0 body-s text-slate-600 leading-relaxed font-medium">
  ...
  return (
    <section id="faq" className="py-20 bg-white scroll-mt-[80px]">
      <div className="max-w-[720px] mx-auto px-4 sm:px-6">
        <h2 className="heading-l text-center mb-16 uppercase tracking-tight text-navy">{t('section.faq.title')}</h2>
