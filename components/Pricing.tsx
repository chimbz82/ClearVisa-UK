...
const Pricing: React.FC<PricingProps> = ({ onStartCheck }) => {
  ...
  return (
    <section id="pricing" className="py-20 bg-[#F5F7FB] scroll-mt-20">
      <div className="app-container">
        <div className="text-center max-w-2xl mx-auto mb-12">
          <span className="text-[11px] font-bold text-slate-500 uppercase tracking-[0.2em] mb-3 block">Pricing</span>
          <h2 className="text-2xl sm:text-3xl font-bold text-[#041229] mb-4">{t('pricing.title')}</h2>
          <p className="text-sm sm:text-base text-slate-600 font-medium">{t('pricing.subtitle')}</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-stretch">
          {PLANS.map((plan) => (
            <div 
              key={plan.id} 
              className={`flex flex-col app-card p-7 sm:p-8 min-h-[320px] relative overflow-hidden transition-all duration-300 ${
                plan.id === 'full' 
