  ...
  return (
    <section id="pricing" className="section-py bg-slate-50/80">
      <div className="app-container">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-8 items-stretch max-w-[1040px] mx-auto">
          {PLANS.map((plan) => (
            <div 
              key={plan.id} 
              className={`flex flex-col app-card transition-all hover:translate-y-[-4px] ${
                plan.id === 'full' ? 'ring-2 ring-accent scale-105 z-10' : 'border border-slate-200'
              }`}
            >
              <div className="mb-10 text-center">
                <span className={`text-caption font-bold ${plan.id === 'full' ? 'text-accent' : 'text-slate-400'}`}>
                  {plan.id === 'basic' ? 'BUDGET' : (plan.id === 'humanReview' ? 'PRO ADD-ON' : 'RECOMMENDED')}
                </span>
                <h3 className="text-h3 mt-2 text-navy h-14 flex items-center justify-center leading-tight">{plan.name}</h3>
                <div className="mt-6 flex items-center justify-center gap-1">
                  <span className="text-h1 text-navy leading-none">£{plan.priceGBP}</span>
                  <span className="text-caption text-slate-400 mt-2 font-bold tracking-widest lowercase">once</span>
                </div>
              </div>
              ...
              <Button 
                ...
                size="md"
              >
                Select {plan.id === 'basic' ? 'Basic' : (plan.id === 'full' ? 'Full' : 'Pro')}
              </Button>
            </div>
          ))}
        </div>

        <div className="max-w-4xl mx-auto bg-white rounded-2xl p-8 mt-16 border border-slate-100 flex flex-col md:flex-row items-center gap-8 shadow-sm">
