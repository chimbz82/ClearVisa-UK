...
const Header: React.FC<HeaderProps> = ({ onStartCheck, onNavigateHome, onScrollToSection }) => {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-200 ${
        ...
        <div className="flex justify-between items-center">
          <div className="flex-shrink-0">
            <button onClick={onNavigateHome} className="flex items-center gap-2 group">
              <div className="w-8 h-8 bg-[#07162A] rounded-lg flex items-center justify-center text-white font-black group-hover:bg-[#16A34A] transition-colors shadow-sm">
              ...
              <span className="text-lg font-bold tracking-tight text-[#07162A]">
                ClearVisa <span className="text-[#16A34A]">UK</span>
              </span>
            </button>
          </div>
          ...
          <div className="hidden lg:flex items-center space-x-1">
            {navLinks.map((link) => (
              <button
                ...
                className="text-[13px] font-medium px-4 py-2 transition-all rounded-full hover:text-[#16A34A] text-slate-600"
              >
                {link.name}
              </button>
            ))}
          ...
          <div className="lg:hidden flex items-center">
            <button onClick={() => setIsMenuOpen(!isMenuOpen)} className="text-[#07162A] p-2 hover:bg-slate-100 rounded-lg transition-colors">
      ...
      {isMenuOpen && (
        <div className="lg:hidden bg-white border-b border-slate-100 absolute w-full left-0 top-full p-6 space-y-3 shadow-xl animate-in slide-in-from-top-4 duration-300">
          {navLinks.map((link) => (
            <button key={link.id} onClick={() => { setIsMenuOpen(false); onScrollToSection(link.id); }} className="block w-full text-left py-3 text-sm font-semibold text-[#07162A] border-b border-slate-50 last:border-0">
              {link.name}
            </button>
          ))}
