...
const Header: React.FC<HeaderProps> = ({ onStartCheck, onNavigateHome, onScrollToSection }) => {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
      isScrolled ? 'bg-white/95 backdrop-blur-md shadow-md py-3 border-b border-slate-200/50' : 'bg-transparent py-5'
    }`}>
      <div className="max-w-6xl mx-auto px-6">
        <div className="flex justify-between items-center h-12">
          <div className="flex-shrink-0">
            <button onClick={onNavigateHome} className="flex items-center gap-2.5 group">
              <div className="w-8 h-8 bg-navy rounded flex items-center justify-center text-white font-black group-hover:bg-accent transition-colors">
              ...
              <span className="text-lg font-bold tracking-tight text-navy">
                ClearVisa <span className="text-accent">UK</span>
              </span>
            </button>
          </div>
          ...
          <div className="hidden lg:flex items-center space-x-1">
            {navLinks.map((link) => (
              <button
                ...
                className="text-[13px] font-bold px-4 py-2 transition-all rounded-full hover:text-accent text-slate-600 hover:bg-slate-50"
              >
                {link.name}
              </button>
            ))}
          ...
          <div className="lg:hidden flex items-center">
            <button onClick={() => setIsMenuOpen(!isMenuOpen)} className="text-navy p-2 hover:bg-slate-100 rounded-lg transition-colors">
      ...
      {isMenuOpen && (
        <div className="lg:hidden bg-white border-b border-slate-100 absolute w-full left-0 top-full p-6 space-y-4 shadow-xl">
          {navLinks.map((link) => (
            <button key={link.id} onClick={() => { setIsMenuOpen(false); onScrollToSection(link.id); }} className="block w-full text-left py-3 text-sm font-bold text-navy border-b border-slate-50 last:border-0">
              {link.name}
            </button>
          ))}
